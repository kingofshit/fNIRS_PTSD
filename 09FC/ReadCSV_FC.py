...
for roi_num in range(0, len(roi_name_list)):
    ...
    roi_channel_count = 0
    for roi_channel in roicsv[roi_name]:
        ...
        for connection_num in range(0, 1128):
            for period_num in range(1, 7):
                ...
                for sub_no in PTSDsub_list:
                    PTSD.append(get_FC(data, PTSDsub_list, HCsub_list, sub_no, connection_num, period_num, oxy_label))
                for sub_no in HCsub_list:
                    HC.append(get_FC(data, PTSDsub_list, HCsub_list, sub_no, connection_num, period_num, oxy_label))
                ...
                t_p_value_array[period_num - 1, connection_num] = t_p_value
            ...
            for period_num in range(1, 7):
                for another_period_num in range(period_num, 7):
                    if period_num == another_period_num:
                        ...
                        for sub_no in PTSDsub_list:
                            PTSDperiod1.append(
                                get_FC(data, PTSDsub_list, HCsub_list, sub_no, connection_num, period_num, oxy_label))
                            PTSDperiod2.append(
                ...
                for roi_channel in roicsv[roi_name]:
                    if not np.isnan(roi_channel):
                        ...
                        for sub_num in range(0, len(sub_list)):
                            ...
                            feature_value = get_FC(data, PTSDsub_list, HCsub_list, sub_no, connection_num, period_num, oxy_label)
                            roi_value_array[
