...
def between_task():
    result_data = pd.read_csv('result_task_channel.csv', dtype=float)
    # 存储PTSD结果
            ...
            if int(oxytype) == plot_oxy_type:
                if PTSD_p_value < 0.05:
                    ...
                    if PTSDperiod1_test_pvalue > 0.05 and PTSDperiod2_test_pvalue > 0.05:
                        ...
                        print('- PTSD配对t检验显著： ' + oxy_label + '_' + str(channel) + '_' + Beta_list[int(beta1)] + '_' + Beta_list[int(beta2)] + ': ' + str(
                            PTSD_p_value))
                ...
                if HC_p_value < 0.05:
                    ...
                    if HCperiod1_test_pvalue > 0.05 and HCperiod2_test_pvalue > 0.05:
                        ...
                        print('- HC配对t检验显著： ' + oxy_label + '_' + str(channel) + '_' + Beta_list[int(beta1)] + '_' + Beta_list[int(beta2)] + ': ' + str(
                            HC_p_value))
...
def between_group():
    ...
    for plot_oxy_type in [1, 2, 3]:
        ...
        plot_t_data = np.zeros((7, 48))
        plot_mannwhitneyu_data = np.zeros((7, 48))
            ...
            if int(oxytype) == plot_oxy_type:
                if t_p_value < 0.05:
                    ...
                    if PTSD_test_pvalue > 0.05 and HC_test_pvalue > 0.05:
                        ...
                        print('- 独立样本t检验显著： ' + oxy_label + '_' + str(channel_name) + '_' + Beta_list[int(beta)] + ': ' + str(t_p_value))
                if mannwhitneyu_p_value < 0.05:
                    oxy_sum[plot_oxy_type - 1, 2] += 1
                    channel_sum[int(channel - 1), 2] += 1
                    beta_sum[int(beta), 2] += 1
                    print('- mannwhitney u检验显著： ' + oxy_label + '_' + str(channel_name) + '_' + Beta_list[int(beta)] + ': ' + str(mannwhitneyu_p_value))
                plot_t_data[int(beta), channel_name - 1] = t_p_value
                plot_mannwhitneyu_data[int(beta), channel_name - 1] = mannwhitneyu_p_value

        # plot_t_roi = np.zeros((7, 48))
        # plot_mannwhitneyu_roi = np.zeros((7, 48))
        # roi_channel_num = 0
        # roi_channel_list = []
